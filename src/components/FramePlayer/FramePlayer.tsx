...
const FramePlayer: React.FC<FramePlayerProps> = ({ frames, fps, handleFpsChange }) => {
  const { currentFrame, isPlaying, play, pause, onFrameChange } = useFramePlayer(frames.length);
  ...
  }, [isPlaying, currentFrame, fps, onFrameChange]);
  ...
  return (
    <div className="frame-player mt-4 max-w-full mx-auto border rounded-lg p-4 bg-white shadow-lg">
      ...
      <ProgressBar currentFrame={currentFrame} totalFrames={frames.length} onFrameChange={onFrameChange} />
    </div>
  );
};
